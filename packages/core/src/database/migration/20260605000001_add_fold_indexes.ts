...
export default {
  id: "20260605000001_add_fold_indexes",
  up(tx) {
    return Effect.gen(function* () {
      yield* tx.run(`ALTER TABLE \`fold\` ADD COLUMN \`start_index\` integer;`)
      yield* tx.run(`ALTER TABLE \`fold\` ADD COLUMN \`end_index\` integer;`)
